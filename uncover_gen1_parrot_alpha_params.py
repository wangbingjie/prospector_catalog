...
sys.path.append(ut_cwd.data_dir('pbeta'))
import emulator as Emu

pdir = ut_cwd.data_dir(data='pirate')
multiemul_file = os.path.join(pdir, "pirate/data/nn/parrot_v4_obsphot_512n_5l_24s_00z24.npy")

# - Parser with default arguments -
parser = prospect.prospect_args.get_parser()
parser.add_argument('--catalog', type=str, default="UNCOVER_v3.0.0_LW_SUPER_CATALOG.fits")
...
def build_model(obs=None, emulfp=multiemul_file, **extras):
    ...
    model_params, fit_order = pfile.params_parrot_alpha(obs=obs)
...
for ifit in np.arange(args.idx0, args.idx1, 1):
    ...
    # if cat[ifit]['use_phot'] == 1: #and cat[ifit]['id'] not in finished_id_list:
    ...
    if _can_fit:
        ...
        hfile = os.path.join(run_params['outdir'], "id_{0}_mcmc_alpha.h5".format(objid))
