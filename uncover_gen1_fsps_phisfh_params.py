...
import dynesty
from pirate.independent_pixels.utils import get_phot
...
parser.add_argument('--catalog', type=str, default=ut_cwd.fcat_phot_latest)
...
parser.add_argument('--idx1', type=int, default=1,
                    help="Range of galaxies to fit, from idx0 to idx1-1; zero-index row number of the catalog.")
parser.add_argument('--outdir', type=str, default='chains_fsps/', nargs='?', const='chains_parrot/',
...
run_params.update({
'free_gas_logu': False, # consistent with parrot
'verbose': True,
'dyn': args.dyn,
...
def load_obs(idx=None, err_floor=0.05, **extras):
    ...
    obs['maggies_unc'] = np.clip(obs['maggies_unc'], obs['maggies']*err_floor, np.inf)
...
for ifit in np.arange(args.idx0, args.idx1, 1):
    ...
    # first check if photometry is good
    # if cat[ifit]['use_phot'] == 1:
    ...
    h5_fname = "id_{}_mcmc_phisfh.h5".format(objid)
    hfile = os.path.join(run_params['outdir'], h5_fname)
    ...
    pp.run_all(h5_fname=h5_fname, fit='fsps_sfhgas', prior='phisfh', mod_fsps=model, sps=sps,
               input_folder=run_params['outdir'], output_folder=run_params['outdir'], 
